...
use App\Services\AnswerGraderIndexService;
use App\Services\HiraganaConverterService;

class DbKanjiAiController extends Controller
{
    public function importExcel2(ExcelSurveyReader $reader)
    {
        ...
        return view('openAI.dbkanjiandai', ['data' => $randomQuestion]);
    ...
    public function score(Request $request, ExcelSurveyReader $reader, AnswerGraderIndexService $grader, HiraganaConverterService $conv){
        $request->validate([
        ...
        $answersAll  =$matched['answers']["all_"] ?? [];

        [$score, $reason, $matched, $sim] = $grader->grade($use_answer, $answersAll, ['fuzzy_threshold' => 0.90]);
        if ($score !== null) {
            ...
            $userHira = $conv->toHiraganaString($use_answer);
            $allHira = $conv->toHiraganaArray($answersAll);
            
            [$score, $reason, $matched, $sim] = $grader->grade($userHira, $allHira, ['fuzzy_threshold' => 0.90]);
