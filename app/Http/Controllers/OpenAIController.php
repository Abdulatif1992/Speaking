...
class OpenAIController extends Controller
{
    ...
    public function scoreOnlyAI(Request $request, ExcelSurveyReader $reader)
    {
        $request->validate([
            'question' => 'required|string',
            'answer' => 'required|string',
        ...
        $predictedGrade = $this->fullPredictOpenAI($incomingQuestion,  $request->input('answer'));     
    ...
    public function importExcel3(ExcelSurveyReader $reader)
    {
        $filePath = storage_path('app/questions.xlsx');
        $sheetName = 'N52';        
        $data = $reader->read($filePath, $sheetName);
        ...
        $randomQuestion = $data[array_rand($data)];
        return view('openAI.designFullAI', ['data' => $randomQuestion]);
    }
